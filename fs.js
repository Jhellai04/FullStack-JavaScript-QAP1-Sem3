...
// WriteFile() method will create a specified filename and fill it with the given input. If the file already exists, it will overwrite that file.

fs.writeFile("test.txt", "Prepare your clothes", function (err) {
  if (err) throw err;
...
fs.appendFile("test.txt", "\nClothes prepared", function (err) {
  if (err) throw err;
  console.log("File has been appended/created");
});

//  We set up the basic interface for the readline module.
const rl = readline.createInterface({
  input: process.stdin,
  output: process.stdout,
});

// We use the unlink method wrapped in a function so that user input can be taken. In this function, the user should be prompted in their terminal asking whether or not the user would like to delete the file.
